...
export function recReadChunksInRange(Buffer, bufferStartIndex, bufferEndIndex, accum) {
    const state = {
        Buffer,
    ...
    if (childContentByteLength > 0) {
        return recReadChunksInRange(Buffer, definitionEndIndex + contentByteLength, bufferEndIndex, {});
    }
    //accumulate siblings
    if (totalChunkEndIndex != bufferEndIndex) {
        return recReadChunksInRange(Buffer, totalChunkEndIndex, bufferEndIndex, accum);
    }
    return accum;
}
//# sourceMappingURL=recReadChunksInRange.js.map